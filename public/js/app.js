...
socket.on('peerDeleted', function (info) {
   ...
   var index = _.findIndex(vm.peers, function (obj) {
       return obj.address == info.address.split('//')[1];
   });
...
socket.on('peerInfo', function (info) {
   ...
   var item = _.find(vm.peers, function (obj) {
       return obj.address == info.address;
   });
